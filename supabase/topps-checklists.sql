...
create or replace function public.touch_topps_updated_at()
returns trigger
language plpgsql
as $$
begin
...
drop trigger if exists trg_topps_pdf_sources_updated_at on public.topps_pdf_sources;
create trigger trg_topps_pdf_sources_updated_at
before update on public.topps_pdf_sources
for each row execute function public.touch_topps_updated_at();

drop trigger if exists trg_topps_sets_updated_at on public.topps_sets;
create trigger trg_topps_sets_updated_at
before update on public.topps_sets
for each row execute function public.touch_topps_updated_at();

drop trigger if exists trg_topps_cards_updated_at on public.topps_checklist_cards;
create trigger trg_topps_cards_updated_at
before update on public.topps_checklist_cards
for each row execute function public.touch_topps_updated_at();
