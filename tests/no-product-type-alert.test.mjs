...
assert.match(dashboard, /<button class="fb" onclick="setF\('no_config',this\)">No Product Type<\/button>/, 'missing the No Product Type quick filter chip');
...
assert.match(dashboard, /if\(activeF==='no_config'\) return mq&&i\.status==='in_stock'&&!i\.configuration;/, 'filterTable must filter to in-stock items with no Product Type when this quick filter is active');
