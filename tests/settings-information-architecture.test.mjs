...
for (const panel of ['vendor-profile-panel','payment-settings-panel','inventory-sync-panel','sync-queue-panel','offline-cache-panel','offline-catalog-manager-panel','spreadsheet-panel','store-members-panel','branding-theme-panel','subscription-panel']) assert.ok(html.includes(panel), `existing panel lost: ${panel}`);
