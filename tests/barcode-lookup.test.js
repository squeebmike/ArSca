...
assert.match(worker,/hint === 'comics' \|\| hint === 'auto'/,'Auto barcode lookup must include Metron comics');
...
assert.match(dashboard,/COVER SUPPLEMENT READ/,'the scanner should show when the comic cover supplement was captured');
assert.match(dashboard,/BROWSE ALL COVERS/,'comic barcode results must offer cover confirmation');
