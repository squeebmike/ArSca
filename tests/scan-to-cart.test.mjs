...
// ── Contract: a QR label may encode a full TCGPlayer/PriceCharting/our-own
// storefront URL (with our own SKU riding along as &wo_sku=) instead of a
// bare SKU -- that URL must be unwrapped back to our SKU before the
// inventory lookup runs. Our storefront fallback omits wo_sku when it
// would just duplicate the item id (to keep the printed QR short/scannable
// -- see labelQrPayload), so the unwrap must also fall back to the item=
// param in that case. A plain (non-URL) value -- what every unmatched
// item's QR/barcode encoded before the storefront fallback existed --
...
assert.match(dashboard, /const params = new URL\(value\)\.searchParams;/, 'a URL value must have its query params parsed');
assert.match(dashboard, /return params\.get\('wo_sku'\) \|\| params\.get\('item'\) \|\| value;/, 'must prefer wo_sku, then fall back to the item= id (the storefront fallback\'s own key), then the raw value');
...
// SKU (or, for our own storefront fallback, the item= id when wo_sku was
// omitted as a duplicate), and passes through anything that is not itself a
// URL (a bare SKU/UPC/ID, or a URL that -- for whatever reason -- carries
// neither param) unchanged. ──
function cartScanLookupValue(value){
  if(!/^https?:\/\//i.test(value)) return value;
  try {
    const params = new URL(value).searchParams;
    return params.get('wo_sku') || params.get('item') || value;
...
assert.equal(
  cartScanLookupValue('https://themanapocket.com/?item=i2'),
  'i2',
  'our own storefront fallback (no wo_sku, since it duplicated the item id) must resolve to the item= id instead'
);
assert.equal(
  cartScanLookupValue('https://themanapocket.com/?item=i2b&wo_sku=UPC-00112233'),
  'UPC-00112233',
  'wo_sku must still win over item= when both are present (a real SKU distinct from the item id)'
);
assert.equal(
  ...
  'a URL with neither wo_sku nor item params must fall through as-is rather than throwing or silently matching the wrong item'
);
