...
// Same IIFE-exposure contract as foc-dashboard.js/card-intake-dashboard.js
// (see tests/foc-dashboard-window-exposure.test.mjs for the original store
...
assert.match(src.trimStart().replace(/^(\/\/[^\n]*\n)+/, ''), /^\(function\(\)\{/, 'this file must be a self-contained IIFE, same as foc-dashboard.js/card-intake-dashboard.js');
...
assert.ok(referenced.size > 8, 'sanity check: this file must reference a substantial number of onclick/onchange handlers');

const exposureBlockStart = src.indexOf('window.ensureDailyTasksPanel');
...
console.log('Daily tasks dashboard window-exposure contract checks passed');
