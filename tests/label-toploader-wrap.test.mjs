...
assert.match(dashboard, /ctx\.imageSmoothingEnabled = true;\s*\n\s*ctx\.drawImage\(textCanvas, 0, 0, w, h\);\s*\n\s*ctx\.imageSmoothingEnabled = false;/, 'the supersampled text layer must be downsampled onto the real canvas WITH smoothing on -- that\'s what actually cleans up the jagged edges -- then smoothing must be turned back off before the code is drawn');
