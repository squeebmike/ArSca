...
assert.match(worker,/PRICECHARTING_OFFLINE_CATEGORIES[\s\S]*'sports'[\s\S]*'comics'[\s\S]*'video_games'[\s\S]*'yugioh'[\s\S]*'one_piece'/);
for(const excluded of ['funko','lego','coins','lorcana','digimon','gaming_magazines']) assert.doesNotMatch(worker,new RegExp(`PRICECHARTING_OFFLINE_CATEGORIES[^;]*'${excluded}'`));
...
for(const category of ['sports','comics','video_games','yugioh','one_piece'])assert.ok(fs.existsSync(path.join(out,category,'manifest.json')),category+' manifest missing');
