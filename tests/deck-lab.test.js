...
assert.match(page,/DEMO · LOCAL ONLY/);assert.match(page,/TODO\(deck-persistence\)/);assert.doesNotMatch(page,/\.from\('(?:decks|deck_cards)'\).*\.(?:insert|upsert)/);assert.match(dashboard,/class="tab primary deck-lab-link" href="mtg-deck-lab\.html">DECK LAB BETA/);assert.match(dashboard,/2026\.07\.08\.01-nav-tabs-lit/);
