...
// Windows-1252 (what actually produced this, not strict Latin-1/ISO-8859-1)
// assigns printable characters -- smart quotes, bullets, em dashes, € -- to
// the 0x80-0x9F byte range, which strict Latin-1 leaves as unprintable
// control codes. A UTF-8 multi-byte character (e.g. U+2019 ’ is bytes E2 80
// 99) reinterpreted through that codepage lands as separate codepoints like
// U+20AC (€) or U+2122 (™), not bytes 0x80-0xFF a plain codePointAt/Latin-1
// reversal can undo. This maps those Windows-1252-only codepoints back to
// their original byte value so the full byte sequence can be reconstructed
// and re-decoded as UTF-8 -- store report: "Fallbacksâ€™" (should be
// "Fallbacks’") and "â€¢" (should be "•") were passing straight through the
// old Latin-1-only version untouched.
...
function repairMojibakeOnce(input) {
  const bytes = [];
  for (const character of input) {
    const point = character.codePointAt(0);
    if (point <= 0xFF) bytes.push(point);
    else if (CP1252_HIGH_BYTES[point] != null) bytes.push(CP1252_HIGH_BYTES[point]);
    else return null; // not a plausible mis-decoded byte sequence -- leave it alone
  }
  try {
...
// PRH's feed has shown up both single-mis-encoded ("FÃ¡bio Moon") and
// double-mis-encoded ("Fallbacksâ€™") text -- the latter went through this
// same UTF-8-as-Windows-1252 reinterpretation twice before we ever saw it.
