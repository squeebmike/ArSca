(function(root,factory){
  ...
  const arenaProvider={id:'arena',site:'MTG Arena',directImport:'yes',confidence:'high',detect:value=>/^\s*Deck\s*$/im.test(String(value||''))&&/^\s*(?:Sideboard|Commander)\s*$/im.test(String(value||'')),normalizeUrl:()=>'',async fetchDeck(){throw manualError(this)},parseExport:text=>String(text||''),getManualInstructions:()=>manual.arena};
