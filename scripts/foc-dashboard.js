...
function renderFocReview(){
  ...
  panel().innerHTML='<section class="foc-hero"><div class="foc-toolbar"><button class="hbtn" onclick="openFocCycle(\''+esc(c.id)+'\')">← COVER WALL</button><button class="hbtn" style="color:var(--g)" onclick="submitPrhOrder()">SUBMIT PRH ORDER</button><button class="hbtn" style="color:var(--red)" onclick="endFocEbayListings()">END REMAINING EBAY LISTINGS</button></div>'+
...
async function endFocEbayListings(){
  if(!state.cycle)return;
  if(!confirm('End every still-live eBay presale listing for FOC '+displayDate(state.cycle.foc_date)+'? This includes covers you DID order from PRH -- use this once you\'re done wanting new eBay presale sales for this cycle (e.g. at/after cutoff, or once a listing has sold what you ordered). Already-sold copies are unaffected; this only stops FURTHER eBay sales.'))return;
  var status=document.getElementById('foc-review-status');
  if(status)status.textContent='Ending eBay listings…';
  try{
    var d=await api('/foc/admin/end-ebay-listings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({storeId:getActiveStoreId(),cycleId:state.cycle.id})});
