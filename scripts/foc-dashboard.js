...
async function archiveCycle(){
  if(!state.cycle||!confirm('Archive FOC '+displayDate(state.cycle.foc_date)+'? It will stop showing on the customer preorder page. This can be undone.'))return;
  try{await api('/foc/admin/cycles?store_id='+encodeURIComponent(getActiveStoreId()),{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({cycleId:state.cycle.id,status:'archived'})});toast_dash('FOC cycle archived');state.loaded=false;await loadCycles(true);}catch(e){toast_dash(e.message);}
}
async function unarchiveCycle(){
  if(!state.cycle)return;
  try{await api('/foc/admin/cycles?store_id='+encodeURIComponent(getActiveStoreId()),{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({cycleId:state.cycle.id,status:'closed'})});toast_dash('FOC cycle restored');await openCycle(state.cycle.id);}catch(e){toast_dash(e.message);}
}
